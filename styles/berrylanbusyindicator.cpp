...
**
****************************************************************************/
#include "berrylanbusyindicator.h"
...
class BerryLanBusyIndicatorNode : public QQuickAnimatedNode
{
public:
    BerryLanBusyIndicatorNode(BerryLanBusyIndicator *item);
...
BerryLanBusyIndicatorNode::BerryLanBusyIndicatorNode(BerryLanBusyIndicator *item)
...
void BerryLanBusyIndicatorNode::updateCurrentTime(int time)
{
    ...
    if (m_penWidht == -1) {
        pen.setWidth(qCeil(size) * m_devicePixelRatio);
    } else {
        pen.setWidth(m_penWidht * m_devicePixelRatio);
    }
    painter.setPen(pen);
...
void BerryLanBusyIndicatorNode::sync(QQuickItem *item)
{
    BerryLanBusyIndicator *indicator = static_cast<BerryLanBusyIndicator *>(item);
    m_color = indicator->color();
    m_width = indicator->width();
    m_height = indicator->height();
    m_penWidht = indicator->penWidth();
    m_devicePixelRatio = indicator->window()->effectiveDevicePixelRatio();
}
BerryLanBusyIndicator::BerryLanBusyIndicator(QQuickItem *parent) :
    QQuickItem(parent)
{
    setFlag(ItemHasContents);
}
QColor BerryLanBusyIndicator::color() const
{
    return m_color;
}
void BerryLanBusyIndicator::setColor(QColor color)
{
    if (m_color == color)
        return;
    m_color = color;
...
bool BerryLanBusyIndicator::isRunning() const
{
    return isVisible();
}
void BerryLanBusyIndicator::setRunning(bool running)
{
    if (running)
        setVisible(true);
}

int BerryLanBusyIndicator::penWidth() const
{
    return m_penWidth;
}

void BerryLanBusyIndicator::setPenWidth(int penWidth)
{
...
int BerryLanBusyIndicator::elapsed() const
{
    return m_elapsed;
}
void BerryLanBusyIndicator::itemChange(QQuickItem::ItemChange change, const QQuickItem::ItemChangeData &data)
{
...
QSGNode *BerryLanBusyIndicator::updatePaintNode(QSGNode *oldNode, UpdatePaintNodeData *)
{
    BerryLanBusyIndicatorNode *node = static_cast<BerryLanBusyIndicatorNode *>(oldNode);
    if (isRunning() && width() > 0 && height() > 0) {
        if (!node) {
            node = new BerryLanBusyIndicatorNode(this);
            node->start();
        }
        node->sync(this);
    } else {
